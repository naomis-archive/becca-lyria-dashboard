...
describe('LeaderboardComponent', () => {
  ...
  it('should render the server name', () => {
    ...
    component.leaderboard = mockData;
    component.serverName = mockData[0].serverName;
    fixture.detectChanges();
  ...
  it('should render the leaderboard', () => {
    ...
    expect(img.getAttribute('src')).toBe(mockData[0].avatar);
    expect(name.textContent.trim()).toBe(mockData[0].userTag);
    expect(experience.textContent.trim()).toBe(
      mockData[0].points.toString() + ' XP'
    );
    expect(level.textContent.trim()).toBe(
      'Level ' + mockData[0].level.toString()
    );
  });
});
